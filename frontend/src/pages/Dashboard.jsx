            ...
            <div className="container mx-auto px-6 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                    ...
                    <Link to="/sermons/new" className="group bg-gradient-to-br from-indigo-600 to-indigo-700 text-white rounded-2xl shadow-lg p-6 flex flex-col justify-between hover:shadow-2xl transition transform hover:-translate-y-1 overflow-hidden relative">
                        ...
                        <div>
                            ...
                            <p className="text-indigo-100 text-sm">{t.dashboard.quickActionDesc}</p>
                        </div>
                    ...
                    {/* Stat Card */}
                    <Link to="/sermons" className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 p-6 flex flex-col justify-between hover:border-indigo-200 dark:hover:border-indigo-900 transition">
                        <div>
                ...
                <section>
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <Link to="/sermons" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 text-sm font-medium hover:underline">{t.dashboard.seeAll}</Link>
                    </div>
                        ...
                        {recentSermons.length > 0 ? recentSermons.map(sermon => (
                            <Link key={sermon.id} to={`/sermons/${sermon.id}`} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 p-5 hover:shadow-md transition group">
                                <div className="flex justify-between items-start mb-3">
                                    <span className="bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 text-xs font-bold px-2 py-1 rounded-md uppercase">{sermon.book} {sermon.chapter}:{sermon.verses}</span>
                                    <span className="text-gray-400 dark:text-gray-500 text-xs">{new Date(sermon.created_at).toLocaleDateString()}</span>
                                </div>
                                <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition truncate">{sermon.theme || 'Sem tema'}</h3>
                                <p className="text-gray-500 dark:text-gray-400 text-sm line-clamp-3 mb-4">{sermon.content.substring(0, 150)}...</p>
                                <div className="text-indigo-500 dark:text-indigo-400 text-sm font-medium flex items-center">
                            ...
                            <div className="col-span-3 text-center py-10 bg-white dark:bg-slate-900 rounded-xl border border-dashed border-gray-300 dark:border-slate-700">
                                ...
                                <Link to="/sermons/new" className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">{t.dashboard.createFirst}</Link>
                            </div>
                        )}
                    </div>
                </section>
